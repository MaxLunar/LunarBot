...
documentation = """colorsort - попиксельное сортирование пикчи."""
    ...
    try:
        ...
        if photo_url is not None:
            ...
            with Image.open(photo_download.raw) as photo:
                a_bytes = photo.getdata()
                orig_len = len(a_bytes)
                c_bytes = list(zip(*[iter(a_bytes)] * multiplier))
                b_bytes = []
                for x in c_bytes:
                    b_bytes.extend(sorted(x, key=lambda z: sum(z)))
                b_bytes.extend([(0, 0, 0)
                                for x in range(orig_len - len(b_bytes))])
                photo.putdata(b_bytes)
